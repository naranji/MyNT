...
corr1T = np.array([])
corr2T = np.array([])
fname = 'perms'+str(n-1)+'.pr'
print fname
permsPrev = pkl.load(open(fname,'rb'))
perms = np.int8(np.zeros([nf,n]))
tempA = range(n)
tempB = np.random.permutation(tempA)
tempA = np.random.permutation(tempA)
k = 0
for ii in range(permsPrev.shape[0]):
    item = permsPrev[ii,:]
    for jj in A:
	jtem = np.insert(item,jj,n-1)
        perms[k,:] += jtem
	k+=1
        c1 = scipy.stats.pearsonr(jtem,tempA)[0]
        c2 = scipy.stats.pearsonr(jtem,tempB)[0]
...
pkl.dump(np.array(perms),open('./perms'+str(n)+'.pr','wb'),pkl.HIGHEST_PROTOCOL)
